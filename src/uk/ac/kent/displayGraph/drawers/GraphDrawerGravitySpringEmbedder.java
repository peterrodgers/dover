...
/**
 * A version of Eades spring embedder for laying out graphs where the newest
 * values for node age have more force downwards.
 * Selected nodes are not moved, but still participate in the
 * force calculation
 *
 * @author Peter Rodgers
 */
public class GraphDrawerGravitySpringEmbedder extends GraphDrawer {
...
/** Maximum age, for working out gravity */
	protected int maxAge = 0;

/** Trivial constructor. */
	public GraphDrawerGravitySpringEmbedder() {
		super(KeyEvent.VK_S,"Spring Embedder");
	}

/** Trivial constructor. */
	public GraphDrawerGravitySpringEmbedder(int key, String s) {
		super(key,s);
	}
/** Constructor. */
	public GraphDrawerGravitySpringEmbedder(int key, String s, boolean inRandomize) {
		super(key,s);
		randomize = inRandomize;
	}
/** Full constructor. */
	public GraphDrawerGravitySpringEmbedder(int key, String s, int mnemomic, boolean inRandomize) {
...
/** Trivial modifier. */
	public void setG(double inG) {g = inG;}
/** Trivial modifier. */
 ...
 */
	public Point2D.Double force(DrawCoord nb) {
		...
		// gravity up for large age, down for low age
		double yGravity = g*(n.getAge()-(maxAge/2));

		double totalXForce = f*(xRepulsive + xAttractive);
		double totalYForce = f*(yRepulsive + yAttractive + yGravity);
